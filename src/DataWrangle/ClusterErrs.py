'''Used to read the previously dumped SrcTrgt pairs, and categorize/cluster them based on compiler errors.
This is done by assigning replacing unique variable names etc in compiler msg with generic "ID".
Then, a column is added in srcTrgt files containing the integer set of clusters (compiler errs) it belongs to'''
...
def writeAllErrs(allErrs):
    name = CF.nameSingleL.split('_')[0]
    f=open(CF.fname_errorIDs, 'w')
    fcsv = csv.writer(f)
    f.write('index,error,count\n')
...
def writeClusterErr(name, dictErrDiff):
    f=open(CF.path_DF + name + "_clusterErrs.csv", 'w')
    fcsv = csv.writer(f)
    ...
    for e in dictErrDiff:
        ...
        fcsv.writerow([s.encode("utf-8") for s in row])
...
def writeErrSets():
    writeErrSetFile(CF.nameSingleL, CF.filenameSrcTrgtPairs_Single)
    writeErrSetFile(CF.nameMultL, CF.filenameSrcTrgtPairs_Mult)
